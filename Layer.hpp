...
private:
    ...
    std::vector<std::vector<double>> weight_velocities;
    std::vector<std::vector<double>> bias_velocities;

    std::vector<double> last_weighted_inputs;
    ...
    void initialize_with_random_weigths()
    {
        for (int out = 0; out < output_nodes_amount; ++out)
        {
            ...
            {
                weights[in][out] = random_double(-3, 3) / input_nodes_amount;
            }
        }
    }
    void update_gradients(std::vector<double> node_derivatives)
    {
        for (int out = 0; out < output_nodes_amount; ++out)
        {
            for (int in = 0; in < input_nodes_amount; ++in)
...
public:
    ...
    std::vector<std::vector<double>> weights; // [from][to]
    std::vector<double> biases;               // added to sum of weights

    std::vector<std::vector<double>> weight_cost_gradient;
    std::vector<double> bias_cost_gradient;
    ...
    Layer(int input_nodes_amount, int output_nodes_amount)
    {
        ...
        weights = std::vector<std::vector<double>>(input_nodes_amount);
        biases = std::vector<double>(output_nodes_amount);
        weight_cost_gradient = std::vector<std::vector<double>>(input_nodes_amount);
        bias_cost_gradient = std::vector<double>(output_nodes_amount);
        for (int i = 0; i < input_nodes_amount; ++i)
        {
            weights[i] = std::vector<double>(output_nodes_amount);
            weight_cost_gradient[i] = std::vector<double>(output_nodes_amount);
        }
        last_weighted_inputs = std::vector<double>(output_nodes_amount);

        initialize_with_random_weigths();
    }
    std::vector<double> calculate_layer_result(std::vector<double> input)
    {
        std::vector<double> output = std::vector<double>(output_nodes_amount);
        for (int out = 0; out < output_nodes_amount; ++out)
        {
            last_weighted_inputs[out] = biases[out];
            for (int in = 0; in < input_nodes_amount; ++in)
            {
                if (!isinf(last_weighted_inputs[out] + weights[in][out] * input[in]))
    ...
    void apply_gradient_descent()
    {
        for (int out = 0; out < output_nodes_amount; out++)
        {
            if (!isinf(biases[out] - bias_cost_gradient[out] * parameters.initial_learning_rate))
                biases[out] -= bias_cost_gradient[out] * parameters.initial_learning_rate;

            for (int in = 0; in < input_nodes_amount; in++)
            {
                if (!isinf(weights[in][out] - weight_cost_gradient[in][out] * parameters.initial_learning_rate))
                    weights[in][out] -= weight_cost_gradient[in][out] * parameters.initial_learning_rate;
            }
        }
    }
    std::vector<double> back_propagation(std::vector<double> out_node_derivatives)
    {
        std::vector<double> in_node_derivatives = std::vector<double>(input_nodes_amount, 0);

        for (int out = 0; out < output_nodes_amount; ++out)
        {
            if (!isinf(out_node_derivatives[out] * parameters.activation.derivate(last_weighted_inputs[out])))
                out_node_derivatives[out] *= parameters.activation.derivate(last_weighted_inputs[out]);

            for (int in = 0; in < input_nodes_amount; ++in)
            {
                if (!isinf(in_node_derivatives[in] + out_node_derivatives[out] * weights[in][out]))
